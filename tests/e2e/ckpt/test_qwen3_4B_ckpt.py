...
def execute(mode: str = "", ckpt_step: int | None = None):
    ...
    if IS_ROCM:
        # ROCm colocate: all 8 GPUs shared by train + rollout, TP=2, CP=2
        actor_gpus = NUM_GPUS
        perf_args = (
    ...
    if IS_ROCM:
        ...
        sglang_args = (
            "--rollout-num-gpus-per-engine 2 --sglang-mem-fraction-static 0.8 --sglang-cuda-graph-bs 1 2 4 8 16 "
        )
